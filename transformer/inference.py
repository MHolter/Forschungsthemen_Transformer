...
# Unangepasst aus der Vorarbeit übernommen
        ...
        for plot in range(25):
            ...
            for index_in, index_tar, _input, target, sensor_number in dataloader:
                ...
                for i in range(forecast_window - 1):
                    ...
                    pos_encoding_old_vals = src[i + 1:, :,
                                            1:]
                    pos_encoding_new_val = target[i + 1, :, 1:].unsqueeze(
                        1)
                    pos_encodings = torch.cat((pos_encoding_old_vals,
                                               pos_encoding_new_val))
                    next_input_model = torch.cat(
                        (src[i + 1:, :, 0].unsqueeze(-1), prediction[-1, :, :].unsqueeze(0)))
                    next_input_model = torch.cat((next_input_model, pos_encodings),
                                                 dim=2)
            ...
            target_position = scaler.inverse_transform(target[:, :, 0].cpu())
            prediction_position = scaler.inverse_transform(all_predictions[:, :, 0].detach().cpu().numpy())
            plot_prediction(plot, path_to_save_predictions, src_position, target_position, prediction_position,
